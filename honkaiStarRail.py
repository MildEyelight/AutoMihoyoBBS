...
class honkaiStarRail:
    def __init__(self) -> None:
        ...
        self.headers['Referer'] = 'https://webstatic.mihoyo.com/bbs/event/signin/hkrpg/index.html?bbs_auth_required=true' \
                                  f'&act_id={setting.honkaiStarRail_Act_id}&utm_source=bbs&utm_medium=mys&utm_campaign=icon'
        ...
        self.account_list = get_account_list("hkrpg_cn", self.headers)
    ...
    def get_checkin_rewards(self) -> list:
        log.info("正在获取签到奖励列表...")
        req = http.get(setting.honkaiStarRail_checkin_rewards, headers=self.headers)
        data = req.json()
        if data["retcode"] != 0:
    ...
    def is_sign(self, region: str, uid: str) -> dict:
        req = http.get(setting.honkaiStarRail_Is_signurl.format(setting.honkaiStarRail_Act_id, region, uid), headers=self.headers)
        data = req.json()
        if data["retcode"] != 0:
            log.warning("获取账号签到信息失败！")
            print(req.text)
            config.config["games"]["cn"]["honkaiStarRail"]["auto_checkin"] = False
            config.save_config()
    ...
    def check_in(self, account):
        ...
        for i in range(max+1):
            ...
            req = http.post(url=setting.honkaiStarRail_Signurl, headers=header,
                            json={'act_id': setting.honkaiStarRail_Act_id, 'region': account[2], 'uid': account[1]})
    ...
    def sign_account(self) -> str:
        ...
        if len(self.account_list) != 0:
            for i in self.account_list:
                if i[1] in config.config["games"]["cn"]["honkaiStarRail"]["black_list"]:
                    continue
                log.info(f"正在为开拓者{i[0]}进行签到...")
                time.sleep(random.randint(2, 8))
                is_data = self.is_sign(region=i[2], uid=i[1])
                print(is_data)
                
                #因为崩铁响应的json中没有first_bind关键字，就默认为false？
                is_data["first_bind"] = False
                if is_data["first_bind"]:
                    log.warning(f"开拓者{i[0]}是第一次绑定米游社，请先手动签到一次")
                else:
                    sign_days = is_data["total_sign_day"] - 1
                    ok = True
                    if is_data["is_sign"]:
                        log.info("签到过了")
                        log.info(f"开拓者{i[0]}今天已经签到过了~\r\n今天获得的奖励是{tools.get_item(self.checkin_rewards[sign_days])}")
                        sign_days += 1
                    else:
                        time.sleep(random.randint(2, 8))
                        req = self.check_in(i)
